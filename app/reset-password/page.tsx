...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    // run once on mount — check URL for code/type
    const processToken = async () => {
      ...
      if (code && type === 'recovery' && !tokenExchanged) {
        try {
          setLoading(true)
          // exchange short-lived code for a session so user can change password
        ...
        // no code in URL — show email input so user can request a reset
        setShowEmailInput(true)
      }
    }
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // keep email input visible to show the message
    } catch (err) {
